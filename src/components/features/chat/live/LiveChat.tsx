...
export function LiveChat({
	...
	const isFollower = followings.some(
		followings => followings.followingId === channel.id
	)
	...
	return (
		<Card
			className={`flex h-[82%] w-[21.5%] flex-col overflow-y-auto lg:fixed xl:mt-0`}
		>
			...
			<CardContent className={`flex h-full flex-col overflow-y-auto p-4`}>
				{isOnline ? (
					...
					>
						...
						<p
							className={`mt-1 w-full text-center text-muted-foreground`}
						>
...
export function LiveChatSkeleton() {
	return (
		<Skeleton
			className={`fixed my-8 flex h-[82%] w-[21.5%] flex-col overflow-y-auto xl:mt-0`}
		/>
	)
}
